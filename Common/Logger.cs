    ...
    public static class Logger
    {
        public static void WriteLogFile(Exception exception)
        {
            try
            {
                ...
                //שינוי הלוגר ב09.24, הפניה לפונקציה החדשה, הלוגר הקודם מטה
        ...
        private static string GetFullPath(string keyName, string fileName)
        {
            ...
            try
            {
                ...
                var currentProjAppSettings = ConfigurationManager.AppSettings;
                string _path;

                if (currentProjAppSettings != null && currentProjAppSettings[keyName] != null)
                {
                    _path = currentProjAppSettings[keyName];
                }
                else
                {
                    string currentAssemblyPath = Assembly.GetExecutingAssembly().Location;
                    _path = Path.GetDirectoryName(currentAssemblyPath); // Get the directory of the DLL
                }


                string path = Path.Combine(_path, Environment.UserName.MakeSafeFilename('_'));
        ...
        internal static void DefineLogger()
        {
            ...
            try
            {
                ...
                // If calling proj's config file was not found, load patholab_common config
